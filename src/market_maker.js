class MarketMaker {
	constructor(stockinfo) {
		...
		this.CONV_WIDTH = this.randomTriangular(0.02, 0.30, 0.10);
		this.alpha = 0.2;
		this.mid = null;
		this.width = null;
	...
	randomTriangular(min, max, mode) {
		return min + Math.random() * (max - min);
	}

	adjustWidth() {
		if (this.width === null) {
			this.width = Math.max(Math.random() * 0.4, 0.02);
		} else {
			this.width = this.alpha * this.randomTriangular(0.02, 2 * this.CONV_WIDTH - 0.02) + (1 - this.alpha) * this.width;
		}
		this.edge = this.width / 2;
		const newPrice = this.stockinfo.stock + Math.random() * (this.edge * 2);
		const oldPrice = this.mid;
		const uptick = newPrice > (this.mid || 0);
		if (this.mid === null) {
			this.mid = this.stockinfo.stock + (Math.random() > 0.5 ? 1 : -1) * Math.random() * 3 * this.edge;
		} else {
			this.mid = this.alpha * newPrice + (1 - this.alpha) * this.mid;
	...
	trade(side, price, size) {
		...
		const alpha = this.normCDF(relSize, 3, 1.5);
